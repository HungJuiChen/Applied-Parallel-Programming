...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    float *device_input_slices;
    cudaMalloc((void**)&device_input_slices, input_slice_size);

    float *device_output_slices;
    cudaMalloc((void**)&device_output_slices, output_slice_size);
    ...
    for (int slice_idx = 0; slice_idx < num_slices; ++slice_idx) {
        ...
        cudaMemcpyAsync(device_input_slices, host_input_slice, current_input_size, cudaMemcpyHostToDevice, streams[slice_idx]);
        ...
        cudaMemcpyAsync(host_output_slice, device_output_slices, current_output_size, cudaMemcpyDeviceToHost, streams[slice_idx]);
    }
    ...
    // Free device memory
    cudaFree(device_input_slices);
    cudaFree(device_output_slices);
