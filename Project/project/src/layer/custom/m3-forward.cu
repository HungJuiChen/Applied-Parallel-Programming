...
__global__ void matrixMultiplySharedAndPermute(const float *A, const float *B, float *C,
                                               int numARows, int numAColumns,
                                               int numBRows, int numBColumns,
                                               int numCRows, int numCColumns,
                                               int Batch, int Map_out,
    ...
    int row = by * TILE_WIDTH + ty;    // Index in the output matrix along the 'Map_out' dimension
    int col = bx * TILE_WIDTH + tx;    // Index in the output matrix along the combined dimension Batch*Height_out*Width_out
    ...
    if (row < numCRows && col < numCColumns) {
        // Now we perform the permutation inline:
        // Currently: row = m (map index), col = b*(Height_out*Width_out) + x
        int image_size = Height_out * Width_out;
        int b = col / image_size;
        int x = col % image_size;
        int m = row;  // same as row (Map_out index)

        // Write the result directly into the permuted output:
        // output layout: [Batch, Map_out, Height_out * Width_out]
        C[b * (Map_out * image_size) + m * image_size + x] = val;
    }
}
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    for(int batch_idx = 0; batch_idx < num_batches; ++batch_idx) {
        ...
        matrixMultiplySharedAndPermute<<<dimGrid, dimBlock>>>(device_mask, unrolled_matrix, device_output + batch_idx * MAX_BATCH_SIZE * Map_out * Height_out * Width_out,
                                                    ...
                                                    current_batch_size, Map_out, Height_out, Width_out);
